...
<nav id="menu" class="tc tw up tc block" class:hidden={!active} class:grid={active}><ul>{#each Nav as [path, name, lang]}<li><a href={$url(path)} class:active={$isActive(path)} on:click={ () => active = !active} hreflang={lang}>{name}</a></li>{/each}</ul></nav>
