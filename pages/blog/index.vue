<template>
  <div class="py-10 px-4 lg:px-10">
    ...
    <ul v-if="articles" id="articles" class="grid gap-6 xl:grid-cols-2 2xl:grid-cols-3 m-0 pl-0" aria-label="List of articles I have written">
      <li v-for="(item, i) in articles" :key="i" class="listItem grid gap-6">
        <div class="published text-sm">
          <Datetime :item="item.createdAt" />
        </div>
        <div class="content">
          <h2 class="leading-3 m-0 text-md font-koho">
            <nuxt-link :to="item.path" class="
								block
								no-underline
								text-title
								hover:text-content
...
export default {
  ...
  data() {
    return {
      title: "Articles",
      description: "Stories about projects and life, easy-to-learn tutorials.",
      url: "/blog/",
    };
  },
  mounted() {
